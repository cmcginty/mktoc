...
"""
This module is the main entry point for the mktoc application. The application
can be run from a script with the following code:

   import mktoc._mktoc as mktoc
   mktoc.main()

For application usage information, run 'pydoc mktoc'.
"""
...
class MkToc(object):
   ...
   def _main(self):
      """Starting execution point. Interprets all program arguments and creates
      a CueParser object to generate a final TOC file."""
      ...
      if opt.wav_files is None:
         ...
         parser = WavParser( opt.wav_files, wav_dir, opt.find_wav,
                             opt.write_tmp)
   ...
   def _banner_msg(self):
      """Returns a TOC comment header that is placed at the top of the TOC
      file."""
   ...
   def _parse_args(self):
      ...
      structure and args list as a tuple. All argument error checking is
...
def main():
   """Starts the program by creating a MkToc object and catches any
   exceptions."""
   global progName
