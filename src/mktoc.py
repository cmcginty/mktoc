...
Usage:
   ...
   CUE_FILE must contain a valid CUE format. When *_FILE is not provided, the
...
Examples:
   ...
   4) Tell mktoc to ignore missing WAV file errors. There is a potential that
      the result TOC file will cause cdrdao to loose pregap information during
      the burn process (see above):

      mktoc -a cue_file.cue

   5) Adjust WAV files for a CD writer offset value. For example, if your CD
   ...
   6) Adjust WAV files for a CD writer offset value, but create new files in
      the /tmp directory:
