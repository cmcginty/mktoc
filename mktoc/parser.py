...
class ParseData(object):
   ...
   def modWavOffset(self,samples,tmp=False):
      ...
      wo = wav.WavOffsetWriter( samples, mktoc.progress_bar.ProgressBar,
...
class _FileLookup(object):
   ...
   def __init__(self, work_dir=os.curdir, find_wav=True):
      ...
      :param work_dir:  Path location of the working directory
      ...
      assert(work_dir)
      self._wav_file_cache = wav.WavFileCache(work_dir)
...
class _CueStateMachine(fsm.StateMachine):
   ...
   def cmd_track( self, match_name, cmd, trk_num, trk_type):
      """Create a new :class:`disc.Track` instance. Change state to 'TRACK'."""
      self.track = disc.Track(int(trk_num))
      self.tracks.append( self.track )
      #TODO: push this into Track class
      if trk_type != 'AUDIO':
         self.track.is_data = True
         self.disc.setMultisession()     # disc is multi-session
      self.change_state( match_handlers=self.track_handlers ) # next state

   def cmd_index( self, match_name, cmd, idx_num, time):
      """
      Create a new :class:`disc.TrackIndex` instance.
...
class CueParser(object):
   ...
   """
   def __init__(self, work_dir=os.curdir, find_wav=True):
      self.file_lookup = _FileLookup(work_dir,find_wav)
   ...
   def parse(self, fh):
      ...
      csm = _CueStateMachine(self.file_lookup)
      return csm( cue )
